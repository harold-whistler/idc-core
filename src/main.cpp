...
// Copyright (c) 2019 The Galileocoin developers
...
#if defined(NDEBUG)
#error "IDChain cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uIDC) are considered zero fee (for relaying and mining)
...
bool CheckZerocoinMint(const uint256& txHash, const CTxOut& txout, CValidationState& state, bool fCheckOnly) {
    if(!fCheckOnly && GetAdjustedTime() < GetSporkValue(SPORK_21_ENABLE_ZEROCOIN))
...
bool CheckTransaction(const CTransaction& tx, bool fZerocoinActive, bool fRejectBadUTXO, CValidationState& state) {
    ...
    if (tx.IsCoinBase()) {
        if (tx.vin[0].scriptSig.size() < 2 || tx.vin[0].scriptSig.size() > 150)
            return state.DoS(100, error("CheckTransaction() : coinbase script size=%d", tx.vin[0].scriptSig.size()), REJECT_INVALID, "bad-cb-length");
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees) {
    ...
    if (!CheckTransaction(tx, GetAdjustedTime() > GetSporkValue(SPORK_21_ENABLE_ZEROCOIN), true, state))
...

    {
        ...
        if(tx.IsZerocoinSpend()) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zIDC spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zIDC spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zIDC spend with serial %s from tx %s is not in valid range\n",
            ...
            if (!view.HaveInputs(tx)) {
                if (fDebug) LogPrintStr("AcceptToMemoryPool: inputs already spent\n");
        ...
        if (!tx.IsZerocoinSpend()) {
            ...
            unsigned int nMaxSigOps = GetAdjustedTime() > GetSporkValue(SPORK_21_ENABLE_ZEROCOIN) ? MAX_TX_SIGOPS_CURRENT : MAX_TX_SIGOPS_LEGACY;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount) {
    ...
    if (nHeight < Params().RAMP_TO_BLOCK()) {
        ret = 0;
    } else if (nHeight <= 15000 && nHeight >= Params().RAMP_TO_BLOCK()) {
        ret = blockValue * 0.45;
    } else if (nHeight <= 57599 && nHeight >= 15000) {
        ret = blockValue * 0.35;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean) {
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from IDChain
...
void ThreadScriptCheck() {
    RenameThread("idchain-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZIDCMinted() {
    int nZerocoinStartHeight = GetZerocoinStartHeight();
...
void RecalculateZIDCSpent() {
    ...
    while (true) {
        ...
        //Remove spends from zIDC supply
...
bool RecalculateIDCSupply(int nHeightStart) {
...
bool ReindexAccumulators(list<uint256>& listMissingCheckpoints, string& strError) {
    ...
    // IDChain: recalculate Accumulator Checkpoints that failed to database properly
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked) {
    ...
    CAmount nValueOut = 0;
    CAmount nValueIn = 0;
    unsigned int nMaxBlockSigOps = block.nTime > GetSporkValue(SPORK_21_ENABLE_ZEROCOIN) ? MAX_BLOCK_SIGOPS_CURRENT : MAX_BLOCK_SIGOPS_LEGACY;
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        if (block.nTime > GetSporkValue(SPORK_22_ZEROCOIN_MAINTENANCE_MODE) && !IsInitialBlockDownload() && tx.ContainsZerocoins()) {
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zIDC with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zIDCSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew) {
    ...
    // If turned on AutoZeromint will automatically convert IDC to zIDC
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) {
    ...
    // Size limits
    unsigned int nMaxBlockSize = GetAdjustedTime() > GetSporkValue(SPORK_21_ENABLE_ZEROCOIN) ? MAX_BLOCK_SIZE_CURRENT : MAX_BLOCK_SIZE_LEGACY;
    ...
    // Check transactions
    bool fZerocoinActive = block.nTime > GetSporkValue(SPORK_21_ENABLE_ZEROCOIN);
    vector<CBigNum> vBlockSerials;
    for (const CTransaction& tx : block.vtx) {
        if (!CheckTransaction(tx, fZerocoinActive, true, state))
            return error("CheckBlock() : CheckTransaction failed");

        // double check that there are no double spent zIDC spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zIDC serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp) {
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zIDC mints and %d zIDC spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived) {
    ...
    if (strCommand == "version") {
        ...
        // IDChain: We use certain sporks during IBD, so check to see if they are
        // available. If not, ask the first peer connected for them.
        if (!pSporkDB->SporkExists(SPORK_14_NEW_PROTOCOL_ENFORCEMENT) &&
                !pSporkDB->SporkExists(SPORK_15_NEW_PROTOCOL_ENFORCEMENT_2) &&
                !pSporkDB->SporkExists(SPORK_22_ZEROCOIN_MAINTENANCE_MODE) &&
                !pSporkDB->SporkExists(SPORK_17_NEW_PROTOCOL_ENFORCEMENT_3) &&
                !pSporkDB->SporkExists(SPORK_18_NEW_PROTOCOL_ENFORCEMENT_4) &&
                !pSporkDB->SporkExists(SPORK_20_NEW_PROTOCOL_DYNAMIC) &&
                !pSporkDB->SporkExists(SPORK_21_ENABLE_ZEROCOIN)) {
